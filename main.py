...
def main(page: ft.Page):
    # A REGRA DA LUMI DA SERICOM: TRY NA PRIMEIRA LINHA!
    try:
        import re
        import base64
        import requests
        ...
        # O botão do PDF foi cortado aqui temporariamente
        tela_boletim = ft.Container(content=ft.Column([ft.Text("Boletins", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), dropdown_aluno_boletim, lista_boletim_notas], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20, visible=False)

        tela_config = ft.Container(content=ft.Column([ft.Text("Configurações", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), ft.Icon(ft.icons.SETTINGS, size=80, color=ROXO_CLARO), ft.Text("Versão Sobrevivência Sericom", color="black", weight=ft.FontWeight.BOLD), ft.ElevatedButton("Sincronizar Banco", icon=ft.icons.SYNC, bgcolor=ROXO_FORTE, color=BRANCO, width=300, on_click=lambda e: carregar_dados_gerais() or mostrar_mensagem("OK!"))], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20, visible=False)
    ...
    # O "BOTE SALVA-VIDAS" DA LUMI DA SERICOM: SE DER ERRO, VAI FICAR VERMELHO NA HORA!
        ...
        page.add(
            ft.Column([
                ...
                ft.Text("Erro Isolado!", color="red", size=24, weight=ft.FontWeight.BOLD),
