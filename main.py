...
def main(page: ft.Page):
    ...
    # --- GERADOR DE PDFs (DE VOLTA E BLINDADO!) ---
    def gerar_pdf_relatorio(nome_aluno, texto_relatorio):
        try:
            from fpdf import FPDF # Isolado aqui pra não pesar na abertura do app!
    ...
    # Ação que chama a geração do PDF do Relatório
    ...
    # Ação que chama a geração do PDF do Boletim
    ...
    # BOTÃO DO PDF VOLTOU PRO RELATÓRIO!
    ...
    # BOTÃO DO PDF VOLTOU PRO BOLETIM!
    ...
    tela_config = ft.Container(content=ft.Column([ft.Text("Configurações", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), ft.Icon(ft.icons.SETTINGS, size=80, color=ROXO_CLARO), ft.Text("Versão Ouro: REST + PDF", color="black", weight=ft.FontWeight.BOLD), ft.ElevatedButton("Sincronizar Banco", icon=ft.icons.SYNC, bgcolor=ROXO_FORTE, color=BRANCO, width=300, on_click=lambda e: carregar_dados_gerais() or mostrar_mensagem("OK!"))], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20, visible=False)
