...
def main(page: ft.Page):
    # O "BOTE SALVA-VIDAS": Se qualquer coisa tentar travar, a tela fica vermelha e conta o porquê!
    try:
        import re
        import base64
        import requests
        ...
        # --- GERADOR DE PDFs (BLINDADO) ---
        def gerar_pdf_relatorio(nome_aluno, texto_relatorio):
            try:
                from fpdf import FPDF # Isolado aqui para não pesar na abertura do app!
        ...
        tela_config = ft.Container(content=ft.Column([ft.Text("Configurações", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), ft.Icon(ft.icons.SETTINGS, size=80, color=ROXO_CLARO), ft.Text("Versão Ouro: Java 17 + REST + PDF", color="black", weight=ft.FontWeight.BOLD), ft.ElevatedButton("Sincronizar Banco", icon=ft.icons.SYNC, bgcolor=ROXO_FORTE, color=BRANCO, width=300, on_click=lambda e: carregar_dados_gerais() or mostrar_mensagem("OK!"))], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20, visible=False)
