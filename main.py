...
def main(page: ft.Page):
    # O ESCUDO FINAL: Se algo tentar quebrar o app, ele mostra a tela vermelha com o erro em vez de fechar!
    try:
        import re
        import base64
        import requests
        ...
        # --- COMUNICAÇÃO REST PURA COM O SUPABASE ---
        ...
        # Funções de Banco de Dados (Imortais)
        ...
        # --- GERADOR DE PDFs (AGORA COM FPDF2) ---
        def gerar_pdf_relatorio(nome_aluno, texto_relatorio):
            try:
                ...
                try:
                    pdf.output(caminho)
                    mostrar_mensagem("PDF salvo nos Downloads do celular!")
                except:
                    pdf.output(nome_arquivo)
                    mostrar_mensagem("PDF gerado com sucesso!")
            except Exception:
        ...
        def gerar_pdf_boletim(nome_aluno, dados_notas):
            try:
                ...
                try:
                    pdf.output(caminho)
                    mostrar_mensagem("Boletim salvo nos Downloads do celular!")
                except:
                    pdf.output(nome_arquivo)
                    mostrar_mensagem("Boletim gerado com sucesso!")
            except Exception:
                mostrar_mensagem("Erro ao criar PDF.", ft.colors.RED_700)

        # --- INTEGRAÇÃO IMGBB (FOTOS) ---
        url_foto_aluno_temp = [""]
        ...
        def foto_joana_selecionada(e: ft.FilePickerResultEvent):
            if e.files and e.files[0].path:
                mostrar_mensagem("Atualizando perfil...", ft.colors.ORANGE_700)
        ...
        # --- VARIÁVEIS GERAIS DA INTERFACE E MODAIS ---
        ...
        def confirmar_edicao_aluno(e):
            ...
            if edit_turma_aluno.value: 
                db_post('joana_turma_alunos', {"id_turma": int(edit_turma_aluno.value), "id_aluno": id_aluno_alvo[0]})
        ...
        dialog_excluir_turma = ft.AlertDialog(title=ft.Text("Excluir?", color="red"), content=ft.Text("Isso não tem volta!"), actions=[ft.TextButton("Cancelar", on_click=fechar_modais), ft.ElevatedButton("Excluir", bgcolor="red", color=BRANCO, on_click=confirmar_exclusao_turma)])
        ...
        dialog_excluir_materia = ft.AlertDialog(title=ft.Text("Excluir?", color="red"), content=ft.Text("Apagar matéria?"), actions=[ft.TextButton("Cancelar", on_click=fechar_modais), ft.ElevatedButton("Excluir", bgcolor="red", color=BRANCO, on_click=confirmar_exclusao_materia)])
        ...
        def abrir_edicao_turma(t):
            id_turma_alvo[0] = t['id']
            edit_nome_turma.value = t.get('nome', '')
            edit_horarios.value = t.get('horarios', '')
            edit_dias.value = t.get('dias', '')
            ...
            if ids:
                ...
                alunos = db_get('joana_alunos', f'id=in.({ids_str})&order=nome_completo.asc')
                for a in alunos: 
                    lista_alunos_na_turma.controls.append(ft.Text(f"• {a['nome_completo']}"))
            dialog_editar_turma.open = True
            page.update()

        def abrir_exclusao_turma(id_t): 
            id_turma_alvo[0] = id_t
            dialog_excluir_turma.open = True
            page.update()

        def abrir_edicao_aluno(a):
            id_aluno_alvo[0] = a['id']
            edit_nome_aluno.value = a.get('nome_completo', '')
            edit_mae_aluno.value = a.get('nome_mae', '')
            edit_nasc_aluno.value = a.get('data_nasc', '')
            edit_tel_aluno.value = a.get('telefone', '')
            edit_turma_aluno.options = dropdown_turma_aluno.options.copy()
            edit_turma_aluno.value = None
            if a.get('foto_url'): 
            ...
            dialog_editar_aluno.open = True
            page.update()

        def abrir_exclusao_aluno(id_a): 
            id_aluno_alvo[0] = id_a
            dialog_excluir_aluno.open = True
            page.update()

        def abrir_edicao_materia(m):
            id_materia_alvo[0] = m['id']
            edit_nome_materia.value = m['nome_materia']
            edit_turma_materia.options = dropdown_turma_materia.options.copy()
            edit_turma_materia.value = str(m['id_turma'])
            dialog_editar_materia.open = True
            page.update()

        def abrir_exclusao_materia(id_m): 
            id_materia_alvo[0] = id_m
            dialog_excluir_materia.open = True
            page.update()

        def criar_linha_turma(t): 
            return ft.Card(content=ft.ListTile(leading=ft.Icon(ft.icons.CLASS_, color=ROXO_CLARO), title=ft.Text(t['nome'], weight=ft.FontWeight.BOLD), subtitle=ft.Text(f"{t.get('dias', '')}"), trailing=ft.Row([ft.IconButton(ft.icons.EDIT, icon_color=ROXO_FORTE, on_click=lambda e: abrir_edicao_turma(t)), ft.IconButton(ft.icons.DELETE, icon_color="red", on_click=lambda e: abrir_exclusao_turma(t['id']))], tight=True)))
        ...
        def criar_linha_materia(m, nome_turma): 
            return ft.Card(content=ft.ListTile(leading=ft.Icon(ft.icons.MENU_BOOK, color=ROXO_CLARO), title=ft.Text(m['nome_materia'], weight=ft.FontWeight.BOLD), subtitle=ft.Text(nome_turma), trailing=ft.Row([ft.IconButton(ft.icons.EDIT, icon_color=ROXO_FORTE, on_click=lambda e: abrir_edicao_materia(m)), ft.IconButton(ft.icons.DELETE, icon_color="red", on_click=lambda e: abrir_exclusao_materia(m['id']))], tight=True)))
        ...
        def carregar_dados_gerais():
            ...
            dropdown_turma_aluno.options.clear()
            dropdown_turma_materia.options.clear()
            dropdown_turma_chamada.options.clear()
            dropdown_turma_notas.options.clear()
            lista_turmas_edit.controls.clear()
            
            for t in resp_turmas:
                opcao = ft.dropdown.Option(key=str(t['id']), text=t['nome'])
                dropdown_turma_aluno.options.append(opcao)
                dropdown_turma_materia.options.append(opcao)
                dropdown_turma_chamada.options.append(opcao)
                dropdown_turma_notas.options.append(opcao)
                lista_turmas_edit.controls.append(criar_linha_turma(t))

            resp_alunos = db_get('joana_alunos', 'order=nome_completo.asc')
            dropdown_aluno_relatorio.options.clear()
            dropdown_aluno_boletim.options.clear()
            lista_alunos_edit.controls.clear()
            
            for a in resp_alunos:
                opcao_aluno = ft.dropdown.Option(key=str(a['id']), text=a['nome_completo'])
                dropdown_aluno_relatorio.options.append(opcao_aluno)
                dropdown_aluno_boletim.options.append(opcao_aluno)
            ...
            lista_turmas_edit.update()
            lista_alunos_edit.update()
            lista_materias_edit.update()
            page.update()

        # --- AÇÕES DO SISTEMA (SALVAR) ---
        def acao_salvar_turma(e):
            ...
            input_nome_turma.value = input_horario_turma.value = input_dias_turma.value = ""
            carregar_dados_gerais() 

        def acao_salvar_aluno(e):
            if not input_nome_aluno.value: return
            dados = {
                "nome_completo": input_nome_aluno.value, 
                "nome_mae": input_mae_aluno.value, 
                "data_nasc": input_nasc_aluno.value, 
                "endereco": input_end_aluno.value, 
                "telefone": input_tel_aluno.value, 
                "foto_url": url_foto_aluno_temp[0]
            }
            res = db_post('joana_alunos', dados)
            if res and dropdown_turma_aluno.value: 
                db_post('joana_turma_alunos', {"id_turma": int(dropdown_turma_aluno.value), "id_aluno": res[0]['id']})
            ...
            preview_foto_aluno.background_image_url = None
            preview_foto_aluno.content = ft.Icon(ft.icons.PERSON, color=BRANCO)
            carregar_dados_gerais()
        ...
        def ao_mudar_turma_chamada(e):
            ...
            ids = [v['id_aluno'] for v in vinculos]
            lista_chamada_alunos.controls.clear()
            controles_chamada_alunos.clear()
            if not ids: 
                page.update()
                return
        ...
        def acao_salvar_frequencia(e):
            ...
            for id_aluno, drop_status in controles_chamada_alunos.items():
                ...
                if verifica: 
                    db_patch('joana_frequencia', 'id', verifica[0]['id'], {"status_presenca": drop_status.value})
                else: 
                    db_post('joana_frequencia', {"id_turma": id_t, "id_aluno": id_aluno, "data": data_sql, "status_presenca": drop_status.value})
            mostrar_mensagem("Chamada salva/atualizada no Banco!")
        ...
        def consultar_historico_chamada(e):
            ...
            if not resp_freq:
                lista_historico_chamada.controls.append(ft.Text("Nenhuma chamada registrada para este dia.", color="red"))
            else:
            ...
            dialog_historico_chamada.open = True
            page.update()
        ...
        def acao_salvar_notas_bimestre(e):
            ...
            id_a = int(dropdown_aluno_notas.value)
            id_m = int(dropdown_materia_notas.value)
            ...
            if verifica: 
                db_patch('joana_notas', 'id', verifica[0]['id'], dados)
            else: 
                db_post('joana_notas', dados)
            
            mostrar_mensagem(f"Salvo com Sucesso! Média final: {round(media, 1)}")
            nota1.value = nota2.value = nota3.value = nota4.value = ""
            page.update()

        # --- LÓGICA DE RELATÓRIOS E BOLETINS ---
        ...
        def carregar_relatorio_existente(e):
            ...
            if resp: 
                texto_relatorio.value = resp[0]['texto_relatorio']
                id_relatorio_atual[0] = resp[0]['id']
            else: 
                texto_relatorio.value = ""
                id_relatorio_atual[0] = None
            page.update()
        ...
        def acao_salvar_relatorio(e):
            ...
            if id_relatorio_atual[0]: 
                db_patch('joana_relatorios', 'id', id_relatorio_atual[0], dados)
            else: 
                db_post('joana_relatorios', dados)
            mostrar_mensagem("Relatório salvo no Banco de Dados!")
        ...
        def carregar_boletim_aluno(e):
            ...
            dados_pdf = []
            if not resp_notas: 
                lista_boletim_notas.controls.append(ft.Text("Nenhuma nota cadastrada.", color="red"))
            else:
                for n in resp_notas:
                    ...
                    card_nota = ft.Card(content=ft.Container(padding=10, content=ft.Column([
                        ft.Text(nome_mat, weight=ft.FontWeight.BOLD, color=ROXO_FORTE, size=16), 
                        ft.Row([ft.Text(f"1º: {n['nota1']}"), ft.Text(f"2º: {n['nota2']}"), ft.Text(f"3º: {n['nota3']}"), ft.Text(f"4º: {n['nota4']}")], alignment=ft.MainAxisAlignment.SPACE_BETWEEN), 
                        ft.Text(f"Média: {n['media']}", weight=ft.FontWeight.BOLD, color="green" if float(n['media']) >= 6 else "red")
                    ])))
                    lista_boletim_notas.controls.append(card_nota)
            
            nome_aluno = next(opt.text for opt in dropdown_aluno_boletim.options if opt.key == str(dropdown_aluno_boletim.value))
            btn_gerar_pdf_boletim.on_click = lambda e: gerar_pdf_boletim(nome_aluno, dados_pdf)
            page.update()
        ...
        nota1 = ft.TextField(label="1º Bim", width=80, border_color=ROXO_FORTE)
        nota2 = ft.TextField(label="2º Bim", width=80, border_color=ROXO_FORTE)
        nota3 = ft.TextField(label="3º Bim", width=80, border_color=ROXO_FORTE)
        nota4 = ft.TextField(label="4º Bim", width=80, border_color=ROXO_FORTE)
        tela_notas = ft.Container(content=ft.Column([ft.Text("Notas e Médias", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), dropdown_turma_notas, dropdown_materia_notas, dropdown_aluno_notas, ft.Row([nota1, nota2, nota3, nota4], alignment=ft.MainAxisAlignment.CENTER), ft.ElevatedButton("Salvar Notas", bgcolor=ROXO_FORTE, color=BRANCO, width=300, on_click=acao_salvar_notas_bimestre)], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20, visible=False)
        ...
        btn_gerar_pdf_boletim = ft.ElevatedButton("Gerar PDF do Boletim", bgcolor="red700", color=BRANCO, width=300, icon=ft.icons.PICTURE_AS_PDF)
        tela_boletim = ft.Container(content=ft.Column([ft.Text("Boletins de Notas", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), dropdown_aluno_boletim, lista_boletim_notas, ft.Divider(), btn_gerar_pdf_boletim], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20, visible=False)

        # Tela 8: Configurações
        tela_config = ft.Container(content=ft.Column([ft.Text("Configurações do Sistema", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), ft.Icon(ft.icons.SETTINGS, size=80, color=ROXO_CLARO), ft.Text("Versão Ouro Absoluta: FPDF2 + Navigation Instantânea", color="black", weight=ft.FontWeight.BOLD), ft.ElevatedButton("Sincronizar Banco", icon=ft.icons.SYNC, bgcolor=ROXO_FORTE, color=BRANCO, width=300, on_click=lambda e: carregar_dados_gerais() or mostrar_mensagem("Banco Sincronizado com Sucesso!"))], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20, visible=False)

        # --- NAVEGAÇÃO IMEDIATA (O SEGREDO CONTRA O LAG) ---
        ...
        def abrir_tela(nome_tela):
            # Passo 1: Muda a tela de forma imediata (Bateu, Levou!)
            for nome, container in telas.items():
                if container: container.visible = (nome == nome_tela)
            page.update() 
            
            # Passo 2: Carrega os dados no banco silenciosamente depois que a tela já abriu
            carregar_dados_gerais()
        ...
        # Carrega os dados silenciosamente na inicialização
        carregar_dados_gerais()

    # TELA VERMELHA DE ALERTA: Em vez de branco, mostra o bug pra gente consertar!
        ...
        page.add(
            ft.Column([
                ...
                ft.Text("Erro Isolado Encontrado!", color="red", size=24, weight=ft.FontWeight.BOLD),
                ft.Text("Mande esse texto pra gente resolver:", color="black", weight="bold"),
