...
def main(page: ft.Page):
    # O ESCUDO FINAL: Mostra o erro na tela em vez de fechar o app!
    try:
        ...
        def foto_edit_selecionada(e: ft.FilePickerResultEvent):
            if e.files and e.files[0].path:
                mostrar_mensagem("Enviando nova foto...", ft.colors.ORANGE_700)
                url = fazer_upload_imgbb(e.files[0].path)
                if url:
                    url_foto_aluno_edit_temp[0] = url
                    preview_edit_foto_aluno.background_image_url = url
                    preview_edit_foto_aluno.content = None
                    mostrar_mensagem("Nova foto pronta!")
                    page.update()
        ...
        def gerar_pdf_boletim(nome_aluno, dados_notas):
            try:
                ...
                mostrar_mensagem("Erro ao criar PDF do Boletim.", ft.colors.RED_700)
        ...
        edit_nasc_aluno = ft.TextField(label="Data Nasc.", border_color=ROXO_FORTE, on_change=aplicar_mascara_data)
        edit_tel_aluno = ft.TextField(label="Telefone", border_color=ROXO_FORTE, on_change=aplicar_mascara_telefone)
        ...
        def confirmar_edicao_aluno(e):
            dados = {"nome_completo": edit_nome_aluno.value, "nome_mae": edit_mae_aluno.value, "data_nasc": edit_nasc_aluno.value, "telefone": edit_tel_aluno.value, "foto_url": url_foto_aluno_edit_temp[0]}
        ...
        dialog_editar_aluno = ft.AlertDialog(title=ft.Text("Editar Aluno", color=ROXO_FORTE), content=ft.Column([ft.Stack([preview_edit_foto_aluno, ft.Container(content=ft.Icon(ft.icons.CAMERA_ALT, color=BRANCO, size=15), bgcolor=ROXO_FORTE, shape=ft.BoxShape.CIRCLE, padding=6, right=0, bottom=0, on_click=lambda _: picker_edit_aluno.pick_files())]), edit_turma_aluno, edit_nome_aluno, edit_mae_aluno, edit_nasc_aluno, edit_tel_aluno], tight=True, horizontal_alignment=ft.CrossAxisAlignment.CENTER), actions=[ft.TextButton("Cancelar", on_click=fechar_modais), ft.ElevatedButton("Salvar", bgcolor=ROXO_FORTE, color=BRANCO, on_click=confirmar_edicao_aluno)])
        ...
        def abrir_exclusao_turma(id_t): 
            id_turma_alvo[0] = id_t; dialog_excluir_turma.open = True; page.update()
        ...
        def abrir_edicao_aluno(a):
            ...
            url_foto_aluno_edit_temp[0] = a.get('foto_url', '')
        ...
        def abrir_exclusao_aluno(id_a): 
            id_aluno_alvo[0] = id_a; dialog_excluir_aluno.open = True; page.update()
        ...
        def abrir_exclusao_materia(id_m): 
            id_materia_alvo[0] = id_m; dialog_excluir_materia.open = True; page.update()
        ...
        def criar_linha_aluno(a): return ft.Card(content=ft.ListTile(leading=ft.CircleAvatar(radius=20, background_image_url=a.get('foto_url') if a.get('foto_url') else None, content=None if a.get('foto_url') else ft.Icon(ft.icons.PERSON), bgcolor=ROXO_CLARO), title=ft.Text(a['nome_completo'], weight=ft.FontWeight.BOLD), subtitle=ft.Text(f"Telefone: {a.get('telefone', '')}"), trailing=ft.Row([ft.IconButton(ft.icons.EDIT, icon_color=ROXO_FORTE, on_click=lambda e: abrir_edicao_aluno(a)), ft.IconButton(ft.icons.DELETE, icon_color="red", on_click=lambda e: abrir_exclusao_aluno(a['id']))], tight=True)))
        ...
        def carregar_dados_gerais():
            try:
                ...
                for m in resp_mat:
                    lista_materias_edit.controls.append(criar_linha_materia(m, mapa_turmas.get(m['id_turma'], 'Desconhecida')))

                page.update()
            except Exception as e:
                print(f"Erro silencioso de conexão: {e}")
        ...
        def acao_salvar_aluno(e):
            ...
            url_foto_aluno_temp[0] = ""; preview_foto_aluno.background_image_url = None; preview_foto_aluno.content = ft.Icon(ft.icons.PERSON, color=BRANCO)
        ...
        def acao_salvar_frequencia(e):
            ...
            mostrar_mensagem("Chamada salva com sucesso!")
        ...
        def acao_salvar_notas_bimestre(e):
            ...
            mostrar_mensagem(f"Salvo! Média final: {round(media, 1)}"); nota1.value = nota2.value = nota3.value = nota4.value = ""; page.update()
        ...
        def acao_salvar_relatorio(e):
            ...
            mostrar_mensagem("Relatório salvo no Banco!")
        ...
        def acao_gerar_pdf_boletim(e):
            ...
            resp_notas = db_get('joana_notas', f'id_aluno=eq.{id_a}')
            materias = {m['id']: m['nome_materia'] for m in db_get('joana_materias')}
            dados_pdf = []
            for n in resp_notas:
        ...
        nota1 = ft.TextField(label="1º", width=60, border_color=ROXO_FORTE); nota2 = ft.TextField(label="2º", width=60, border_color=ROXO_FORTE); nota3 = ft.TextField(label="3º", width=60, border_color=ROXO_FORTE); nota4 = ft.TextField(label="4º", width=60, border_color=ROXO_FORTE)
        tela_notas = ft.Container(content=ft.Column([ft.Text("Notas e Médias", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), dropdown_turma_notas, dropdown_materia_notas, dropdown_aluno_notas, ft.Row([nota1, nota2, nota3, nota4], alignment=ft.MainAxisAlignment.CENTER), ft.ElevatedButton("Salvar Notas", bgcolor=ROXO_FORTE, color=BRANCO, width=300, on_click=acao_salvar_notas_bimestre)], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20)
