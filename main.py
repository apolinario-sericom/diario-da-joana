...
def main(page: ft.Page):
    ...
    def foto_joana_selecionada(e: ft.FilePickerResultEvent):
        if e.files and e.files[0].path:
            ...
            if url:
                ...
                mostrar_mensagem("Foto de perfil atualizada!")
                page.update()
            else: mostrar_mensagem("Erro na foto.", ft.colors.RED_700)
    ...
    def foto_aluno_selecionada(e: ft.FilePickerResultEvent):
        if e.files and e.files[0].path:
            ...
            if url:
                ...
                mostrar_mensagem("Foto pronta para salvar no cadastro!")
                page.update()
            else: mostrar_mensagem("Erro na foto.", ft.colors.RED_700)
    ...
    # --- AÇÕES DOS MODAIS (TURMA, ALUNO, MATERIA) ---
    ...
    def confirmar_edicao_aluno(e):
        try:
            dados = {
                "nome_completo": edit_nome_aluno.value, 
                "nome_mae": edit_mae_aluno.value, 
                "data_nasc": edit_nasc_aluno.value, 
                "telefone": edit_tel_aluno.value
            }
            ...
            fechar_modais()
            mostrar_mensagem("Erro ao atualizar.", ft.colors.RED_700)
    ...
    def confirmar_exclusao_aluno(e):
        try:
            ...
            fechar_modais()
            mostrar_mensagem("Erro ao excluir.", ft.colors.RED_700)
    ...
    def confirmar_exclusao_materia(e):
        try:
            ...
            carregar_dados_gerais()
        except: mostrar_mensagem("Erro ao excluir.", ft.colors.RED_700)

    # --- ESTRUTURAS DOS MODAIS ---
    ...
    dialog_excluir_turma = ft.AlertDialog(title=ft.Text("Excluir Turma?", color="red"), content=ft.Text("Esta ação não tem volta!"), actions=[ft.TextButton("Cancelar", on_click=fechar_modais), ft.ElevatedButton("Excluir", bgcolor="red", color=BRANCO, on_click=confirmar_exclusao_turma)])
    ...
    dialog_excluir_aluno = ft.AlertDialog(title=ft.Text("Excluir Aluno?", color="red"), content=ft.Text("Apagar do sistema?"), actions=[ft.TextButton("Cancelar", on_click=fechar_modais), ft.ElevatedButton("Excluir", bgcolor="red", color=BRANCO, on_click=confirmar_exclusao_aluno)])
    ...
    dialog_excluir_materia = ft.AlertDialog(title=ft.Text("Excluir Matéria?", color="red"), content=ft.Text("Apagar do sistema?"), actions=[ft.TextButton("Cancelar", on_click=fechar_modais), ft.ElevatedButton("Excluir", bgcolor="red", color=BRANCO, on_click=confirmar_exclusao_materia)])

    lista_historico_chamada = ft.Column(scroll=ft.ScrollMode.AUTO, height=300)
    dialog_historico_chamada = ft.AlertDialog(title=ft.Text("Histórico do Dia", color=ROXO_FORTE), content=lista_historico_chamada, actions=[ft.TextButton("Fechar", on_click=fechar_modais)])
    ...
    def acao_salvar_aluno(e):
        ...
        try:
            ...
            mostrar_mensagem("Aluno salvo com sucesso!")
            ...
            carregar_dados_gerais()
        except Exception as e: print(e); mostrar_mensagem("Erro ao salvar aluno.", ft.colors.RED_700)
    ...
    def ao_mudar_turma_chamada(e):
        ...
        try:
            ...
            for aluno in supabase.table('joana_alunos').select('id, nome_completo, foto_url').in_('id', ids).order('nome_completo').execute().data:
                drop_status = ft.Dropdown(options=[ft.dropdown.Option("P", "Presente"), ft.dropdown.Option("F", "Falta"), ft.dropdown.Option("J", "Justificada")], value="P", width=120, dense=True)
    ...
    def acao_salvar_frequencia(e):
        ...
        try:
            ...
            mostrar_mensagem("Chamada salva/atualizada com sucesso!")
        except: mostrar_mensagem("Erro ao salvar.", ft.colors.RED_700)
    ...
    def acao_salvar_notas_bimestre(e):
        ...
        try:
            ...
            mostrar_mensagem(f"Notas salvas/atualizadas! Média: {round(media, 1)}")
            nota1.value = nota2.value = nota3.value = nota4.value = ""; page.update()
        except: mostrar_mensagem("Erro nas notas.", ft.colors.RED_700)

    # --- LÓGICA RELATÓRIOS E BOLETINS ---
    ...
    def carregar_boletim_aluno(e):
        ...
        try:
            ...
            dados_pdf = []
            if not resp_notas.data: lista_boletim_notas.controls.append(ft.Text("Nenhuma nota lançada.", color="red"))
            else:
    ...
    tela_chamada = ft.Container(content=ft.Column([ft.Text("Chamada", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), ft.Row([dropdown_turma_chamada, input_data_chamada], alignment=ft.MainAxisAlignment.CENTER), ft.ElevatedButton("Ver Histórico do Dia", icon=ft.icons.HISTORY, on_click=consultar_historico_chamada), ft.Divider(), lista_chamada_alunos, ft.ElevatedButton("Salvar Chamada", bgcolor="green700", color=BRANCO, width=300, on_click=acao_salvar_frequencia)], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20, visible=False)
    ...
    texto_relatorio = ft.TextField(label="Cole o texto aqui...", border_color=ROXO_FORTE, bgcolor=BRANCO, multiline=True, min_lines=5)
    ...
    tela_config = ft.Container(content=ft.Column([ft.Text("Configurações", size=24, weight=ft.FontWeight.BOLD, color=ROXO_FORTE), ft.Icon(ft.icons.SETTINGS, size=80, color=ROXO_CLARO), ft.Text("Diário da Joana - Premium 13.1", color="black", weight=ft.FontWeight.BOLD), ft.ElevatedButton("Sincronizar Dados", icon=ft.icons.SYNC, bgcolor=ROXO_FORTE, color=BRANCO, width=300, on_click=lambda e: carregar_dados_gerais() or mostrar_mensagem("Sincronizado!"))], horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=20, visible=False)
